"""hermes-petpi — Local-only variant (no external API keys).

Uses whisper.cpp + piper-tts for fully offline voice interaction.
Connects to local Hermes Agent / Ollama / llama.cpp for LLM inference.
"""
...
logging.basicConfig(
    ...
    datefmt="%H:%M:%S",
    handlers=[logging.StreamHandler(), logging.FileHandler(config.LOG_FILE, mode="a")],
)
log = logging.getLogger("hermes-petpi-local")

from display import Display
from record_audio import Recorder, check_audio_level
from stt_local import transcribe
from tts_local import TTSPlayer
from local_client import stream_response
...
class LocalAssistant:
    def __init__(self):
        ...
        self._tts = TTSPlayer() if config.ENABLE_OFFLINE_TTS else None
    ...
    def _on_button_press(self):
        ...
        if self._tts:
            ...
            self.display.set_status("Listening...", color=(140, 200, 255), subtitle="Speak now", accent_color=(60, 140, 255))
        try:
            self.recorder.start()
    ...
    def _process_utterance(self):
        ...
        try:
            ...
            self.display.stop_spinner()
            if not self._is_stale(my_gen) and self.ptt.state in (State.TRANSCRIBING, State.THINKING, State.STREAMING):
                self._go_idle()
    ...
    def _process_utterance_inner(self, my_gen: int):
        ...
        if rms < 300:
            ...
            if self._is_stale(my_gen):
                return
            self.display.stop_character()
            self.display.set_status("No speech detected", color=(160, 160, 160), subtitle="Try again", accent_color=(80, 80, 80))
            time.sleep(1.5)
            if not self._is_stale(my_gen):
                self._go_idle()
            return
        ...
        if self._tts:
            self.display.set_character_state("transcribing")
        else:
            self.display.set_status("Transcribing...", color=(255, 230, 100), subtitle="One moment", accent_color=(255, 180, 0))
...
", tts_buffer))
        ...
        self._dismiss.clear()
        self._dismiss.wait(timeout=config.RESPONSE_HOLD_TIMEOUT)
    ...
    def _show_error(self, msg: str):
        ...
        self.display.stop_character()
        self.display.set_status(msg[:50] + ("..." if len(msg) > 50 else ""), color=(255, 120, 120), subtitle="Something went wrong", accent_color=(200, 0, 0))
        time.sleep(3)
        self._go_idle()
    ...
    def run(self):
        ...
        try:
            while not self._shutdown.is_set():
                ...
                if not self.display.is_sleeping and self.ptt.state == State.IDLE and not worker_busy and time.monotonic() - self._last_idle_refresh > 30:
                    self.display.set_idle_screen()
                    self._last_idle_refresh = time.monotonic()
                if not self.display.is_sleeping and self.ptt.state == State.IDLE and not worker_busy and time.monotonic() - self._last_activity > config.SLEEP_TIMEOUT:
